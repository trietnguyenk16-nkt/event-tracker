...
export default function Home() {
  ...
  const [aiDraft, setAiDraft] = useState<Partial<EventItem> | null>(null);
  ...
  const [q, setQ] = useState<{ date: string; tag: string; status: 'all' | 'todo' | 'done' }>({ date: '', tag: '', status: 'all' });
  ...
  useEffect(() => {
    ...
    fetch('/api/events')
      ...
      .then(payload => { setEvents(payload.data ?? payload); setOffline(false); setStatusMessage(''); })
      .catch(() => { setMockMode(true); setOffline(!navigator.onLine); if (!cached) setEvents(mockEvents); setStatusMessage('Không kết nối được database · đang hiển thị bản xem trước an toàn.'); })
  ...
  const filtered = useMemo(() => filterEvents(events, { ...q, calendar: calendarFilter, calendars: calendarVisibility, eventCalendars }), [events, q, eventCalendars, calendarFilter, calendarVisibility]);
  ...
  async function save(payload: Partial<EventItem>) {
    try {
      ...
      setStatusMessage(editing ? 'Đã cập nhật sự kiện.' : 'Đã tạo sự kiện mới.');
    } catch {
      ...
      setMockMode(true);
      setStatusMessage('Chưa lưu được lên server · đã giữ bản nháp trên thiết bị.');
      setEvents(current => editing ? current.map(event => event.id === item.id ? item : event) : [item, ...current]);
      setEventCalendars(current => ({ ...current, [item.id]: calendarFilter === 'all' ? 'Cá nhân' : calendarFilter }));
    }
    setOpen(false); setEditing(null); setAiDraft(null);
  }
  ...
  function openCreate() { setEditing(null); setAiDraft(null); setOpen(true); }
  ...
  return <main className="min-h-[100dvh] overflow-x-hidden bg-paper pb-[env(safe-area-inset-bottom)] text-ink dark:bg-[#111820] dark:text-paper">
    ...
    {open && <EventForm initial={editing ?? aiDraft} onSave={save} onClose={() => { setOpen(false); setEditing(null); setAiDraft(null); }} />}
  </main>;
}
